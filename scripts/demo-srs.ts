...
for (let i = 1; i <= 5; i++) {
  ...
  console.log(
    `  review ${i}: nextIntervalDays=${result.nextIntervalDays} | isLapse=${result.isLapse}`,
  );
}
...
console.log(
  `  Starting: mastery=${wordD.masteryCount} | phase=${wordD.phase} | lapseCount=${wordD.lapseCount}`,
);
...
const bucketLabel = (wordId: string): string => {
  if (wordId === carryOver1.wordId || wordId === carryOver2.wordId)
    return '[carry-over]';
...
console.log(
  `  Pool: ${pool.length} words → batch: ${batchE.batchSize} questions`,
);
for (const q of batchE.questions) {
  console.log(
    `  ${bucketLabel(q.wordId).padEnd(18)} wordId=${q.wordId.padEnd(12)} type=${q.type}`,
  );
}
console.log(
  `  Distribution: mc=${batchE.distributionBreakdown.mc} | wordBlock=${batchE.distributionBreakdown.wordBlock} | audio=${batchE.distributionBreakdown.audio}`,
);
...
console.log(
  `  mc              : ${String(batchWithAudio.distributionBreakdown.mc).padStart(6)}                 ${String(batchWithoutAudio.distributionBreakdown.mc).padStart(6)}`,
);
console.log(
  `  wordBlock       : ${String(batchWithAudio.distributionBreakdown.wordBlock).padStart(6)}                 ${String(batchWithoutAudio.distributionBreakdown.wordBlock).padStart(6)}`,
);
console.log(
  `  audio           : ${String(batchWithAudio.distributionBreakdown.audio).padStart(6)}                 ${String(batchWithoutAudio.distributionBreakdown.audio).padStart(6)}`,
);
console.log(
  `  total           : ${String(batchWithAudio.batchSize).padStart(6)}                 ${String(batchWithoutAudio.batchSize).padStart(6)}`,
);

// ── Scenario G: Active window slot calculation ───────────────────────────────

header(
  'Scenario G: Active window slot calculation (activeWordLimit=5, newWordsPerBatch=3)',
);
...
const activeWord2 = promoteToReview(
  makeWord('gracias', 'curated'),
  windowConfig,
);
...
header(
  'Scenario H: Stuck word detection + shelving (shelveAfterBatches=5, maxShelved=2)',
);
...
console.log(
  `    stuck.length   = ${resultH.stuck.length}  (batchesSinceLastProgress >= 5)`,
);
console.log(
  `    toShelve       = [${resultH.toShelve.map((w) => w.wordId).join(', ')}]`,
);
...
const preShelved1: WordState = shelveWord(
  { ...makeWord('seis', 'curated'), batchesSinceLastProgress: 5 },
  86_400_000,
);
const preShelved2: WordState = shelveWord(
  { ...makeWord('siete', 'curated'), batchesSinceLastProgress: 5 },
  86_400_000,
);
const thirdStuck: WordState = {
  ...makeWord('ocho', 'curated'),
  batchesSinceLastProgress: 5,
};
...
console.log(
  `    toShelve       = [${resultHCap.toShelve.map((w) => w.wordId).join(', ')}]  (newest stuck word)`,
);
...
header(
  'Scenario J: Continuous wrong rule (3 consecutive wrongs → mastery reset)',
);
...
console.log(
  `  After 4 correct: mastery=${wordJ.masteryCount} | phase=${wordJ.phase}`,
);
...
wordJ2 = applyFoundationalWrongRule(wordJ2, config);
console.log(
  `  After 2 wrongs: mastery=${wordJ2.masteryCount} | consecutiveWrongCount=${wordJ2.consecutiveWrongCount ?? 0}`,
);
wordJ2 = updateMastery(wordJ2, true, config);
console.log(
  `  After correct:  mastery=${wordJ2.masteryCount} | consecutiveWrongCount=${wordJ2.consecutiveWrongCount ?? 0}`,
);
...
const allocActive = getFoundationalAllocation(
  allocConfig.batchSize,
  activePool,
  allocConfig,
);
console.log(`  Active pool (2 below threshold, 1 above):`);
console.log(
  `    slots        = ${allocActive.slots}   (Math.round(10 × 0.2) = 2)`,
);
...
const allocDepleted = getFoundationalAllocation(
  allocConfig.batchSize,
  depletedPool,
  allocConfig,
);
...
console.log(
  `    slots        = ${allocDepleted.slots}   (Math.round(10 × 0.05) = 1)`,
);
