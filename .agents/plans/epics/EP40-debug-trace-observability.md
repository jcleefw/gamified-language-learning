...
**Type**: Epic Plan
**Depends on**: EP37 (Learning-authority — server transition channel + correlation-id plumbing) · Review-mode redesign (merged — the Revision authority path `POST /api/reviews/answer` + `review_answer_events` this epic extends) · Logger ADR `20260708T143342Z` (complete)
**Parallel with**: N/A
**Predecessor**: Review-mode redesign (merged — established the Revision authority this epic brings to Learning's replay parity)
...
- **Review-mode redesign (merged)** — the Revision authority path (`POST /api/reviews/answer`,
  `review_answer_events`, the `child`-logger + `x-correlation-id` read) that ST05 brings up to
  `answer_events` replay fidelity.
