...
2. Run:
   `.agents/tools/archive-epic.sh backfill`
   It reports `<EP_NUMBER>: compact_pr=<result>` for every epic with a
   `compact_pr: null` story (report only — it does not write to `index.json`).
3. For each epic whose result is an integer, re-append every story of that
   epic via `archive-append.sh --story - --replace` with `compact_pr` set to
   that number. Do not hand-edit `index.json`.
4. If the result is `undetermined`, leave `compact_pr: null` for that epic's
   stories — do not guess a value from any other source.
5. Report a summary: which epics were resolved (with PR number) and which
   remain undetermined.

## Rules

- `compact_pr` is only ever set from `archive-epic.sh backfill` output —
