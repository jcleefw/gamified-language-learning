# EP25-DS04: Sentence State & Spacing in srs-demo Vue App

**Date**: 20260614T230313Z
**Status**: Draft
...
DS03 implemented sentence scheduling in the CLI demo (`demo/learning-io.ts`): `SentenceState`, eligibility gates (`active` + batch-gap), and streak-based graduation/shelving. The Vue app (`apps/srs-demo`) currently has none of this — `App.vue` only assembles MCQ word questions and `QuizCard.vue` renders a "coming soon" stub for `word-block` questions.

DS04 ports the sentence layer into the Vue session loop and adds a `WordBlock` renderer to `QuizCard`. Three design decisions were resolved before stories were written (see §6).
