# EP17-DS02: Consumer Migration and Package Cleanup Specification
...
**Tasks**:
- [ ] Repoint every `@gll/srs-engine-v2` and `@gll/srs-shelving` import to the correct sub-path
- [ ] Drop the `@gll/srs-shelving` dependency from `package.json`

**Acceptance Criteria**:
- [ ] `apps/srs-demo` has zero imports from `@gll/srs-engine-v2/review`
- [ ] `pnpm --filter srs-demo typecheck && test` pass
...
**Tasks**:
- [ ] Repoint every `@gll/srs-engine-v2`, `@gll/srs-review`, `@gll/srs-shelving` import in both apps to the correct sub-path
- [ ] Drop the now-redundant `@gll/srs-review` / `@gll/srs-shelving` dependencies from both `package.json`

**Acceptance Criteria**:
- [ ] Neither app depends on `@gll/srs-review` or `@gll/srs-shelving` in `package.json`
- [ ] `pnpm --filter server --filter cli-demo-db typecheck && test` pass
...
**Tasks**:
- [ ] Repoint imports to the correct sub-path
- [ ] Update `package.json` dependencies if either package listed `@gll/srs-review` or `@gll/srs-shelving` directly

**Acceptance Criteria**:
- [ ] `pnpm --filter db --filter api-contract typecheck && test` pass
...
**Tasks**:
- [ ] Delete `packages/srs-review/` and `packages/srs-shelving/` directories
- [ ] Run install to regenerate the lockfile / workspace graph

**Acceptance Criteria**:
- [ ] `grep -r "srs-review\|srs-shelving" packages/ apps/` returns no hits outside historical changelogs/ADRs
- [ ] Full repo `typecheck` and `test` pass with the packages removed
