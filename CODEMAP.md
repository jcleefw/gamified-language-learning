...
| `scripts/quiz-runner.ts` | Terminal quiz runner — HTTP API client; calls `/seed` → `/batch` → `/answers`; single-batch interactive MC quiz with raw keypress input; run via `pnpm quiz` |
| `tsconfig.json`          | Root TypeScript config — covers `scripts/` with `@types/node`; not part of turbo pipeline |
