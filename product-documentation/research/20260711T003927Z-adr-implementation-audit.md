...
- **Historical**: cross-referenced `.agents/changelogs/EP*` to confirm a decision was *actually shipped*, not just coded speculatively. Changelog EP-numbers are cited as evidence below.
- **NOT done**: no tests run, no typecheck, no app driven. A "Built ✅" here means *code exists and matches the decision's shape, and a changelog records the work* — it does **not** certify the code passes its tests or behaves correctly at runtime. Rows needing behavioural proof are flagged.

Confidence tiers used: **High** = read the logic + changelog; **Med** = symbol/table present + changelog; **Doc-diff** = ADR-vs-file comparison I read directly (highest certainty of the *discrepancy*).
...
| ContentStore & deck-document (07-06) | **Proposed** ⚠ | ✅ shipped | `SqliteContentStore`, `decks.doc`, `DeckDocSchema`; **EP35** | High |
| Async storage contract (07-06) | **Proposed** ⚠ | ✅ shipped | both stores `Promise<…>`, `no-floating-promises` lint; **EP34** | High |
| SRS review packaging & rating (07-08) | Accepted | ✅ | `review_answer_events`; EP36 | Med |
| srs-demo learning authority + debug trace (07-08) | Accepted | ✅ | server-authoritative `/api/answer`; trace composables; EP37/EP40 | Med |
...
| Agentic memory hook (03-04) | **Proposed** | Partially | `.agents/memory/` used; hook mechanics governance-side, out of product scope. |
...
| Quiz contract & answer authority (03-13) | **Proposed** | ✅ Built | EP15 shipped server-authoritative answers. Status never flipped. |
...
| composeWordBatch boundary (05-12) | **Proposed** | ✅ Built | EP23/EP25 composer registry. Status never flipped. |
...
| Sentence corpus ingestion (05-14) | **Proposed** | ✅ Built (as document import) | Now flows through ContentStore `importCurriculum`; see §4.1. Status never flipped. |
...
The 06-20 schema ADR bills itself *"the complete schema for all domains in a single authoritative document."* It is now out of date:
...
**Precedence**: ContentStore (07-06) + later event-log work win. **Action**: add an "Amended by ContentStore ADR (07-06); event-log tables added post-hoc" banner to the 06-20 ADR, or explicitly demote it and name `schema.ts` the source of truth. *(Evidence: `schema.ts` read in full; EP35 changelog.)*
...
| §5 api-contract | *"No runtime dependencies — types only"* | Ships **`zod`** as a runtime dependency | `api-contract/package.json:23` |
...
### 4.5 Recurring hygiene issue: "Proposed" ADRs that shipped
Six ADRs are still **"Proposed"** despite being built and changelog-recorded: ContentStore (07-06, EP35), Async storage (07-06, EP34), Quiz contract (03-13, EP15), composeWordBatch (05-12, EP23/25), Sentence corpus ingestion (05-14), plus the governance-side memory hook (03-04). **Action**: batch-flip to Accepted (or Superseded where relevant). This is the single cheapest fix to restore ADR-status trustworthiness.
...
- **Retention metric — "% of learned words retained"** — the *stated success metric* of the review redesign, but no definition or instrumentation exists yet. Biggest open item. `review_answer_events` was built as the seed data for it. (Idea brief Known-Unknowns; review-ahead OQ-C.)
...
1. **Flip the six "Proposed"→built ADRs to Accepted** (§4.5). Cheapest restore of doc trust. *(Doc-only.)*
2. **Reconcile the Database Schema ADR (06-20)** with the document model + event tables, or demote it (§4.1). *(Doc-only.)*
3. **Reconcile the API Surface ADR (03-05)** to the shipped envelope/namespace and mark "types-only" superseded by the ContentStore zod decision (§4.2). *(Doc-only.)*
...
_Evidence base: `schema.ts`, `apps/server/src/{app.ts,routes/*,config/*,identity/*,seed/*}`, `packages/{db,api-contract,logger,srs-*}/src`, `apps/srs-demo/src`, and `.agents/changelogs/EP13,EP15,EP23,EP25,EP26,EP32,EP34,EP35,EP36,EP37,EP38,EP39,EP40,EP41`. No runtime/tests executed — see §1._
