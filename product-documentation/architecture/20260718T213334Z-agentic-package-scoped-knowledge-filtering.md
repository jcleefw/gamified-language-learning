...
This ADR keeps the problem write-time-shaped and builds only what the one live consumer (`archive-epic` compaction) needs. The data it authors — path-notated aspect tags, a curated alias map, a per-unit blacklist — is graph-ready, so the read-time consumer ([Graph RAG Read Model](20260720T235931Z-engineering-graph-rag-read-model.md)) adds a *reader* without reshaping anything built here.
...
- **D11 — Build write-time only; one mechanism spans apps and packages.** The same per-unit declaration serves write-time suppression (live consumer: compaction) and, later, read-time filtering (consumer: Graph RAG — graph edges from paths, the alias map as query-synonym resolver, ryoiki-scoped queries), which is decided in the [Graph RAG Read Model](20260720T235931Z-engineering-graph-rag-read-model.md) ADR. No read-side lookup, filter, or registry now. No app-vs-package structural difference: every unit gets ryoiki-as-headings + a central per-unit blacklist + include-by-default.
...
**Deferred (read-time slice — decided in the [Graph RAG Read Model](20260720T235931Z-engineering-graph-rag-read-model.md) ADR):** ingestion expanding paths into graph edges (deferred there too, D6); the alias map as query-synonym resolver (D5); read-time filtering by ryoiki (D4). Phase-1 artifacts need no reshape for this.
...
- ADR: [Graph RAG Read Model](20260720T235931Z-engineering-graph-rag-read-model.md) — the deferred read-time consumer this ADR points to; it decides how the blacklist (D4), alias map (D5), and slash-path hierarchy (D6) are consumed at read time
