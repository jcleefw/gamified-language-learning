...
describe('detectStuckWords', () => {
  describe('detection threshold', () => {
    ...
    it('flags words at exactly threshold (3 batches)', () => {
      const words = [makeWord({ wordId: 'stuck-1', batchesSinceLastProgress: 3 })]
      const result = detectStuckWords(words, baseConfig)
  ...
  describe('shelving capacity & max-2 cap', () => {
    it('returns all stuck words in toShelve when none shelved and capacity available', () => {
      ...
      expect(result.toShelve.map((w) => w.wordId)).toEqual(['stuck-1', 'stuck-2'])
...
describe('shelveWord', () => {
  it('sets shelvedUntil to now + durationMs', () => {
    ...
    expect(shelved.shelvedUntil!.getTime()).toBeGreaterThanOrEqual(now + durationMs)
    expect(shelved.shelvedUntil!.getTime()).toBeLessThanOrEqual(now + durationMs + 100) // allow small timing variance
  })
  ...
  it('works with different durations', () => {
    ...
    expect(shelved1h.shelvedUntil!.getTime()).toBeGreaterThanOrEqual(now + 3600000)

    const shelved7d = shelveWord(word, 604800000)
    expect(shelved7d.shelvedUntil!.getTime()).toBeGreaterThanOrEqual(now + 604800000)
  })
})
