...
describe('composeBatch — question type distribution', () => {
  ...
  it('assigns correct number of each question type in questions array', () => {
    ...
    const mcCount = batch.questions.filter((q): boolean => q.type === 'mc').length
    const wordBlockCount = batch.questions.filter((q): boolean => q.type === 'wordBlock').length
    const audioCount = batch.questions.filter((q): boolean => q.type === 'audio').length
...
describe('composeBatch — audio redistribution', () => {
  it('redistributes audio slots to MC when audioAvailable is false', () => {
    ...
    const batchWithAudio = composeBatch(words, configWith30, { audioAvailable: true })
    const batchWithoutAudio = composeBatch(words, configWith30, { audioAvailable: false })
    ...
    expect(batchWithoutAudio.distributionBreakdown.mc).toBe(
      batchWithAudio.distributionBreakdown.mc + batchWithAudio.distributionBreakdown.audio
    )

    // Total should remain the same
    expect(
      batchWithoutAudio.distributionBreakdown.mc + batchWithoutAudio.distributionBreakdown.wordBlock
    ).toBe(
  ...
  it('defaults to audioAvailable=true when option not provided', () => {
    ...
    const batchExplicit = composeBatch(words, configWith20, { audioAvailable: true })

    expect(batchDefault.distributionBreakdown.audio).toBe(batchExplicit.distributionBreakdown.audio)
    expect(batchDefault.distributionBreakdown.mc).toBe(batchExplicit.distributionBreakdown.mc)
  })
  ...
  it('assigns audio types to actual questions only when audioAvailable=true', () => {
    ...
    const batchWithAudio = composeBatch(words, baseConfig, { audioAvailable: true })
    const batchWithoutAudio = composeBatch(words, baseConfig, { audioAvailable: false })

    const audioQuestionsWithAudio = batchWithAudio.questions.filter((q): boolean => q.type === 'audio')
    const audioQuestionsWithoutAudio = batchWithoutAudio.questions.filter((q): boolean => q.type === 'audio')
  ...
  it('maintains batchSize when redistributing audio', () => {
    ...
    const batchWithAudio = composeBatch(words, configWith25, { audioAvailable: true })
    const batchWithoutAudio = composeBatch(words, configWith25, { audioAvailable: false })

    expect(batchWithAudio.batchSize).toBe(batchWithoutAudio.batchSize)
    expect(batchWithAudio.questions.length).toBe(batchWithoutAudio.questions.length)
  })
})
