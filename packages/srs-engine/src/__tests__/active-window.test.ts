...
describe('getEligibleWords', () => {
  describe('active window filtering', () => {
    it('returns empty active array when no words in review phase', () => {
      const words = [makeWord({ phase: 'learning' }), makeWord({ phase: 'learning' })]
      const result = getEligibleWords(words, baseConfig)
      expect(result.active).toHaveLength(0)
    })

    it('returns words in srsM2_review phase as active', () => {
      const reviewWord = makeWord({ wordId: 'review-1', phase: 'srsM2_review' })
      const learningWord = makeWord({ wordId: 'learning-1', phase: 'learning' })
    ...
    it('correctly identifies multiple active words', () => {
      ...
      expect(result.active.map((w) => w.wordId)).toEqual(['review-1', 'review-2'])
    })
  })
  ...
  describe('immutability', () => {
    it('does not mutate input array', () => {
      const words = [makeWord({ phase: 'learning' }), makeWord({ phase: 'srsM2_review' })]
