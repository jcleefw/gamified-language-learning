...
describe('FsrsScheduler', () => {
  describe('scheduleReview', () => {
    ...
    it('correct answers produce increasing intervals across sequential reviews', () => {
      ...
      const stateAfterFirst = makeWordState({
        fsrsState: result1.updatedFsrsState,
      });
      const result2 = scheduler.scheduleReview(stateAfterFirst, true);

      expect(result2.nextIntervalDays).toBeGreaterThanOrEqual(
        result1.nextIntervalDays,
      );
    });
    ...
    it('wrong answer produces a shorter interval than correct answer', () => {
      ...
      expect(wrongResult.nextIntervalDays).toBeLessThan(
        correctResult.nextIntervalDays,
      );
    });
