...
import { updateMastery, getActiveFoundationalWords, applyFoundationalWrongRule } from '../../src/index.js'
...
function makeLearningWord(wordId: string, category: WordState['category'] = 'foundational'): WordState {
  return {
    wordId,
    category,
    masteryCount: 0,
...
describe('foundational-lifecycle integration', () => {
  it('foundational words partially mastered via updateMastery are counted as active', () => {
    ...
    const result = getActiveFoundationalWords([partial1, partial2, promoted], config)
  ...
  it('active limit reports availableSlots=0 when 3+ foundational words are in learning', () => {
    ...
    const result = getActiveFoundationalWords([word1, word2, word3, word4], config)
