...
import { updateMastery, composeBatch, getFoundationalAllocation } from '../../src/index.js'
...
describe('foundational-allocation-lifecycle integration', () => {
  it('active pool returns poolDepleted=false and 20% slots when some foundational words are below threshold', () => {
    ...
    const foundAbove  = promoteToReview(makeLearningWord('tres', 'foundational'))

    const result = getFoundationalAllocation(config.batchSize, [foundBelow1, foundBelow2, foundAbove], config)
  ...
  it('depleted pool returns poolDepleted=true and 5% slots when all foundational words are past threshold', () => {
    ...
    const result = getFoundationalAllocation(config.batchSize, depletedPool, config)
  ...
  it('depletion transition shifts allocation from 20% to 5% when remaining foundational words are promoted past threshold', () => {
    ...
    const activeResult = getFoundationalAllocation(config.batchSize, [promoted1, word2], config)
    ...
    const depletedResult = getFoundationalAllocation(config.batchSize, [promoted1, promoted2], config)
