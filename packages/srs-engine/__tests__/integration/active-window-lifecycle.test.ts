...
import { updateMastery, getEligibleWords, detectStuckWords, shelveWord, unshelveWord, isShelved } from '../../src/index.js'
...
function makeLearningWord(wordId: string, category: WordState['category'] = 'curated'): WordState {
  return {
    wordId,
    category,
    masteryCount: 0,
...
describe('active-window-lifecycle integration', () => {
  it('words promoted via updateMastery are counted as active by getEligibleWords', () => {
    ...
    const result = getEligibleWords([active1, active2, learning1, learning2, learning3], config)
  ...
  it('newSlots decreases as active window fills to activeWordLimit', () => {
    const windowConfig: SrsConfig = { ...config, activeWordLimit: 4, newWordsPerBatch: 3 }
    ...
    const resultPartial = getEligibleWords([active1, active2, learning1, learning2], windowConfig)
    ...
    const resultFull = getEligibleWords([active1, active2, active3, active4, learning1], windowConfig)
  ...
  it('detectStuckWords identifies words at or above the shelveAfterBatches threshold', () => {
    const shelveConfig: SrsConfig = { ...config, shelveAfterBatches: 3, maxShelved: 2 }

    const stuckWord1: WordState = { ...makeLearningWord('uno', 'foundational'), batchesSinceLastProgress: 3 }
    const stuckWord2: WordState = { ...makeLearningWord('dos', 'foundational'), batchesSinceLastProgress: 5 }
    const almostStuck: WordState = { ...makeLearningWord('tres', 'foundational'), batchesSinceLastProgress: 2 }
    const fineWord: WordState   = { ...makeLearningWord('cuatro', 'foundational'), batchesSinceLastProgress: 0 }

    const result = detectStuckWords([stuckWord1, stuckWord2, almostStuck, fineWord], shelveConfig)
  ...
  it('shelveWord + isShelved + unshelveWord state transitions work end-to-end', () => {
    ...
    const expiredWord: WordState = { ...word, shelvedUntil: new Date(Date.now() - 1000) }
