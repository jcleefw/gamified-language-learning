...
import { initAdaptiveSession, advanceAdaptiveSession } from '../adaptive-session.js';
import type { QuizItem } from '../compose-word-batch.js';

describe('initAdaptiveSession', () => {
  const mockWords: QuizItem[] = [
    { id: 'w1', native: 'n1', romanization: 'r1', english: 'e1', type: 'word', language: 'th' },
    { id: 'w2', native: 'n2', romanization: 'r2', english: 'e2', type: 'word', language: 'th' },
    { id: 'w3', native: 'n3', romanization: 'r3', english: 'e3', type: 'word', language: 'th' },
    { id: 'w4', native: 'n4', romanization: 'r4', english: 'e4', type: 'word', language: 'th' },
  ];
  ...
  it('clones initialRunState to prevent cross-deck mutation', () => {
    const initialRunState = new Map([['w1', { wordId: 'w1', seen: 1, correct: 1, mastery: 1, correctStreak: 1, wrongStreak: 0 }]]);
    const state = initAdaptiveSession(mockWords, config, new Set(), initialRunState);
...
describe('advanceAdaptiveSession', () => {
  ...
  it('ensures immutability of runState and sessionRetryCounts', () => {
    ...
    nextState.runState.set('w1', { ...nextState.runState.get('w1')!, seen: 99 });
