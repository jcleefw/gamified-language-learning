...
│  └─ frontmatter (unit, sources, updated) + ## ryoiki headings    │
...
│  └─ stories[] carry (domain, ryoiki, epic, pr) — folded into a   │
│     provenance index keyed by (domain, ryoiki)                   │
│                                                                   │
...
│     ├─ byRyoiki:  (domain, ryoiki) -> { stories, epics, prs }   │
│     └─ epicSpan:   epicId -> set of ryoiki keys it produced      │
│                                                                   │
│  2. ingestKnowledge(graph, root, filter, provenance)              │
│     ├─ domain node per KNOWLEDGE.md    (id = `unit` frontmatter)   │
│     ├─ ryoiki node per ## heading     (content = prose beneath)   │
│     │    + provenance stamped from the index (sources/epics/prs)  │
│     ├─ domain  --contains--> ryoiki                              │
│     └─ ryoiki --relates-->  ryoiki   (cross-domain, shared epic) │
│                                                                   │
│  Stories and epics are NEVER nodes — only citations on ryoikis.  │
└──────────────────────────┬────────────────────────────────────────┘
                           │
                           ▼
┌─────────────────────────────────────────────────────────────────┐
...
| --- | --- | --- |
| `contains` | domain → ryoiki | a domain groups its ryoikis |
| `relates` | ryoiki → ryoiki | two ryoikis in *different* domains produced by the same epic (co-evolution); label is `via <epicId>` |
| `decides` | adr → ryoiki \| domain | the ADR's `**Decides:**` field names the knowledge it governs |
| `supersedes` | adr → adr | a newer ADR replaces/amends an older one (auto-parsed from `Superseded by` / `Amended by` links) |

Ryoikis within one domain are already grouped by their shared domain node, so no
intra-domain `relates` edges are drawn.

## Provenance matching

A story's `ryoiki` field is a slug (`app-shell`); a `KNOWLEDGE.md` heading is a
title (`App Shell`). They are matched by `normalizeRyoiki` (lowercase, strip
non-alphanumerics), keyed with the story's `domain`. A story with no `ryoiki` (or
...
- `tracks`: keep only provenance from stories on these tracks (e.g. `['project']`
  drops `agentic`). Ryoiki nodes still come from `KNOWLEDGE.md`; the filter only
