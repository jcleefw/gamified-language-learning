...
defineProps<{
  // Local unlock gate (any word mastered). Gates both modes.
...
<template>
  <div class="review-hub">
    ...
    <div class="mode-cards">
      ...
      >
        <span class="mode-title">
          ...
          <span
            ...
            class="review-badge review-badge--error"
            >—</span
          >
      ...
      <!-- Practice Anytime — all learned words, on demand. No badge (not due-scoped). -->
      <button
        class="mode-card anytime-card"
        :class="{ locked: !reviewUnlocked }"
        :disabled="!reviewUnlocked"
        @click="reviewUnlocked && emit('anytime')"
      >
        <span class="mode-title">
          Practice Anytime
          <span v-if="!reviewUnlocked" class="review-lock">🔒</span>
        </span>
        <span class="mode-desc">
          <template v-if="!reviewUnlocked"
            >Locked until you master a word.</template
          >
          <template v-else
            >Practise any learned word, even if it isn't due.</template
          >
        </span>
      </button>
    </div>
