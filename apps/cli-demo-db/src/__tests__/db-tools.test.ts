...
describe('clearUserState', () => {
  ...
  it('removes all word and sentence state for the given userId', () => {
    const store = new SqliteLearningStore(db);
    store.upsertWordState('cli-user', {
      wordId: 'test-id',
      seen: 1,
      correct: 1,
      mastery: 1,
    ...
    const wordStates = store.getAllWordStates('cli-user');
    expect(wordStates.size).toBe(0);
  });

  it('does not remove state for a different userId', () => {
    const store = new SqliteLearningStore(db);
    store.upsertWordState('other-user', {
      wordId: 'test-id',
      seen: 1,
      correct: 1,
      mastery: 1,
    ...
    const otherStates = store.getAllWordStates('other-user');
...
describe('seedDb', () => {
  ...
  it('baseline: leaves zero word state rows', () => {
    seedDb('baseline', db);
    const rows = db.select().from(schema.user_word_states).all();
    expect(rows.length).toBe(0);
  });

  it('mid-session: inserts 4 word states with correct mastery values', () => {
    seedDb('mid-session', db);
    const rows = db.select().from(schema.user_word_states).all();
  ...
  it('sentence-ready: inserts 6 word states all with seen >= 2', () => {
    seedDb('sentence-ready', db);
    const rows = db.select().from(schema.user_word_states).all();
  ...
  it('is idempotent — calling twice gives same row count', () => {
    seedDb('mid-session', db);
  ...
  it('throws for unknown fixture name', () => {
    expect(() => seedDb('nonexistent-fixture', db)).toThrowError('nonexistent-fixture');
  });
});
